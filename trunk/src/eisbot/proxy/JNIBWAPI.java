...
//	private HashSet<Integer> enemyIDs = new HashSet<Integer>();
	...
	public native String getUpgradeTypeName(int upgradeID);
	...
	public native int[] getCreepSpread(); 

	// unit commands: http://code.google.com/p/bwapi/wiki/Unit
	public native boolean attackMove(int unitID, int x, int y); 
	public native boolean attackUnit(int unitID, int targetID);
	...
	public native boolean cancelTrain(int unitID);
...
//	// squad management
//	public native void createSquad(int squadID, int style);
//	public native void deleteSquad(int squadID);
//	public native void setSquadStyle(int squadID, int style);
//	public native void addUnit(int unitID, int squadID);
//	public native void deleteUnit(int unitID, int squadID); 
//  
//	public native void squadMoveTile(int ID, int x, int y);
//	public native void squadHold(int ID);
//	public native void squadAttack(int ID, int targetID);
//	public native void squadAttackTile(int ID, int x, int y);
	...
	public native void drawTerrain(boolean enable);
	public native void enableUserInput();
	public native void enablePerfectInformation();
	public native void setGameSpeed(int speed); 
	public native void setCameraLocation(int x, int y);	
	public native void pingMinimap(int x, int y);	
		...
		try {
			...
			for (int index=0; index<unitData.length; index += Unit.numAttributes) {
				...
				if (unit.getTypeID() == UnitTypes.Resource_Vespene_Geyser.ordinal()) {
					...
					Player player = players.get(unit.getPlayerID());				
					if (player.isSelf()) {
	 ...
	 * C++ callback function.
	 * 
	 * Notifies the event listener that StarCraft is still in menu mode.
	 */ 
	private void menuUpdate(int frameCount) {
		listener.menuUpdate(frameCount);
	}
	...
	private void gameUpdate() {
		 ...
		 try {
			...
			self.updateResearch(this, getResearchStatus(), getUpgradeStatus());
	...
	public ArrayList<PlayerScore> playerScores() {
		...
		int[] playerData = getPlayerScores();
		for (int index = 0; index<playerData.length; index+=PlayerScore.numAttributes) {
